    ...
    <main class="flex-1 flex flex-col font-['Poppins'] bg-[#D4E6B5] min-h-screen">
        <header class="topbar p-6 shadow-md">
            <div class="flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-semibold text-[#877B66]">Ubah Profil Owner</h1>
                    <p class="text-sm text-gray-700">Perbarui informasi akun Anda</p>
                </div>
        ...
        <section class="p-8">
            <div class="bg-white p-6 rounded-xl shadow max-w-3xl mx-auto">
                ...
                <form action="{{ route('owner.profil.update') }}" method="POST" class="space-y-6">
                    @csrf
                    <div>
                        <label for="nama" class="block text-sm font-medium text-[#877B66] mb-1">Nama</label>
                        <input type="text" name="nama" class="form-input w-full border-gray-300 rounded-lg shadow-sm focus:ring-[#AFC97E] focus:border-[#AFC97E]" value="{{ $owner->nama }}" required>
                    </div>
                    <div>
                        <label for="no_hp" class="block text-sm font-medium text-[#877B66] mb-1">No HP</label>
                        <input type="text" name="no_hp" class="form-input w-full border-gray-300 rounded-lg shadow-sm focus:ring-[#AFC97E] focus:border-[#AFC97E]" value="{{ $owner->no_hp }}" required>
                    </div>
                    <div>
                        <label for="email" class="block text-sm font-medium text-[#877B66] mb-1">Email</label>
                        <input type="email" name="email" class="form-input w-full border-gray-300 rounded-lg shadow-sm focus:ring-[#AFC97E] focus:border-[#AFC97E]" value="{{ $owner->akun->email }}" required>
                    </div>
                    <div>
                        <label for="username" class="block text-sm font-medium text-[#877B66] mb-1">Username</label>
                        <input type="text" name="username" class="form-input w-full border-gray-300 rounded-lg shadow-sm focus:ring-[#AFC97E] focus:border-[#AFC97E]" value="{{ $owner->akun->username }}" required>
                    </div>
                    <div>
